...
import { syncCart, clearCart } from '../../../lib/redux/slices/cartSlice';
import SkeletonCheckout from '@/components/ui/SkeletonCheckout';
...
const CartItem = React.memo(({ item }) => (
  <div className="flex items-center gap-4 py-4 border-b border-gray-100 dark:border-gray-700 last:border-0">
    ...
    <div className="flex-1">
      <h3 className="font-semibold text-gray-800 dark:text-amber-100">
        {item.title}
      </h3>
      <p className="text-sm text-gray-600 dark:text-gray-400">
        تعداد: {item.quantity} × {item.price.toLocaleString('fa-IR')} تومان
      </p>
    </div>
    <p className="font-bold text-lg text-gray-800 dark:text-amber-100">
...
export default function CheckoutPage() {
  ...
  const [shippingMethod] = useState({ name: 'پست پیشتاز', cost: 45000 });
  ...
  const validateForm = () => {
    ...
    else if (!/^09\d{9}$/.test(formData.phone)) errors.phone = 'شماره نامعتبر';
    return errors;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="container mx-auto px-4 py-6">
        <Breadcrumb items={['shop', { name: 'سبد خرید', href: '/shop/cart' }, 'تسویه حساب']} />
      </div>
      ...
      <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-white py-12">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl opacity-90">لطفاً اطلاعات ارسال را وارد کنید</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12 max-w-6xl">
        {!cart || cart.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-2xl shadow-lg">
            ...
            <Link
              href="/shop"
              className="inline-block bg-amber-500 text-white px-8 py-4 rounded-full font-bold hover:bg-amber-600 transition"
            >
              بازگشت به فروشگاه
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                {/* فرم‌ها... (همون قبلی) */}
                {/* فقط برای کوتاه شدن حذف کردم، ولی تو کد اصلی باشه */}
              </form>
            </div>

            {/* جزئیات سفارش */}
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <h2 className="text-2xl font-bold mb-8">جزئیات سفارش</h2>
              ...
              <button
                ...
                className="w-full mt-8 bg-gradient-to-r from-green-500 to-green-600 text-white py-5 rounded-2xl font-bold text-xl hover:shadow-2xl transition disabled:opacity-70"
              >
                {isConfirming ? 'در حال پردازش...' : 'تأیید و پرداخت'}
              </button>
            </div>
          </div>
        )}
